...
TASK = (
    "Analyze this framework's Python source tree and produce a Markdown "
    "report named codebase_analysis.md. The report MUST include: "
    "(a) count of Python files (recursive; find_files auto-skips "
    "__pycache__ / .venv / dist / build); "
    "(b) total lines of code across those files; "
    "(c) top 5 files by line count as a table with columns 'path' and "
    "'lines'; "
    "(d) count of `class ` definitions and `def ` definitions across "
    "the whole tree; "
    "(e) any TODO / FIXME / XXX comments — list each with file:line "
    "and the actual comment text (or write 'none found' if there are "
    "none). Every number must come from an actual tool call — no "
    "estimates. When saving, use write_file with if_exists='rename' "
    "so a prior run's report doesn't block this one."
)
...
def build_explorer(llm):
    ...
    Uses AgentType.Few_Shot — the template ships worked tool-composition
    examples + an explicit "PERSIST across turns" example. Verbose is
    ON here so you can watch the tool trace in real time; the ReAct
    thought field isn't emitted by Few_Shot (that's a ReAct-only
    field), so debugging is via tool.call events rather than reasoning
    lines. max_iterations bumped to 50 because exploration tasks can
    legitimately need many read/grep calls; the framework's duplicate-
    call guard is the safety net against spirals."""
    return AgentRunner(
        model=llm,
        agent=AgentType.Few_Shot,
    ...
    supervisor = Supervisor(
        ...
        agents={
            ...
            "explorer": (
                ...
                "Tools: find_files (by glob), grep (by regex), "
                "read_path (single file). CANNOT run Python. CANNOT "
                "write files. CANNOT compute LOC / totals / rankings / "
                "AST metrics — those need a `code`-capable specialist "
                "(SPAWN one). Best for: locate files, extract raw text "
                "matches, sample individual files.",
                build_explorer(llm),
            ),
            "reporter": (
                "Write / edit files in ./workspace only. Tools: "
                "write_file, edit_file. CANNOT run Python. CANNOT read "
                "anything outside ./workspace. Best for: writing the "
                "final Markdown deliverable, appending sections, fixing "
                "typos in an already-produced file.",
                build_reporter(llm),
            ),
        },
        max_subtasks=8,
