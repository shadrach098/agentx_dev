...
# Loads a `state` dict from the pickle file if it exists, else an empty
# dict. Uses a defensive try/except so a corrupted or version-mismatched
# pickle doesn't crash the whole snippet — the model sees a clean
# empty state and can rebuild.
...
try:
    if _agentx_os.path.exists(_AGENTX_STATE_PATH):
        with open(_AGENTX_STATE_PATH, "rb") as _agentx_f:
            state = _agentx_pickle.load(_agentx_f)
        if not isinstance(state, dict):
            state = {{}}
    else:
        state = {{}}
except Exception as _agentx_e:
    # Corrupted pickle from a prior crash / version bump. Start fresh
    # rather than propagate the error into the user's code.
    print(f"[run_python.state] warning: failed to load state ({{_agentx_e}}); starting fresh", flush=True)
    state = {{}}
'''
...
try:
    ...
    with open(_AGENTX_STATE_PATH, "wb") as _agentx_f:
        _agentx_pickle.dump(_agentx_clean, _agentx_f)
    if _agentx_dropped:
